class V4::UsersController < V4::BaseController
  ...
  def index
    ...
    if stale?(@users)
      render json: @users, include: included_rels, meta: pagination_meta(@users), adapter: :json_api
    end
  end
  ...
  def show
    if stale?(@user)
      render json: @user, include: [:active_boards, :archived_boards], adapter: :json_api
    end
  end
